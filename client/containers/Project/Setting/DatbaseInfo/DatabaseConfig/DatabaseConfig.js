...
import ProjectEnvContent from '../DatbaseConfigContent/ConfigContent.js';
...
class DatabaseConfig extends Component {
  ...
  setCurConfig(envId) {
    let curConfig = {};
    if (!this.state.allDatabaseConfig) {
      this.setState({
        curConfig: curConfig
      });
      return;
    ...
    for (let i = 0; i < this.state.allDatabaseConfig.length; i++) {
      if (this.state.allDatabaseConfig[i].env_id && this.state.allDatabaseConfig[i].env_id == envId) {
        curConfig = this.state.allDatabaseConfig[i];
        break;
      }
    }
    this.setState({
  ...
  async getAllConfigData() {
    let result = await axios.get('/api/databaseInfo/all?project_id=' + this.props.projectId);
    if (result.data.errcode === 0) {
      if (result.data.data) {
        this.setState({
          allDatabaseConfig: result.data.data
        });
      }
    }
  }
  ...
  async onSave(assignValue) {
    await axios.post('/api/databaseInfo', assignValue).then(res => {
          ...
          <Layout className="env-content">
            <Content style={{ background: '#fff', padding: 24, margin: 0, minHeight: 280 }}>
              <ProjectEnvContent
                ...
                configData={this.state.curConfig}
